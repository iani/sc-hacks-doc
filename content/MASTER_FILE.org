...
*** Future plans: Audio Feedback between synths
    ...
    A particularly interesting case is when an Ndef reads and processes its own output.  An implementation of this feature, using [[http://doc.sccode.org/Classes/InFeedback.html][InFeedback]] is being tested.
...
: 2 player-linking operators
#+BEGIN_SRC javascript
\source @>.in \effect;
\effect <@.out \source;
#+END_SRC

In addition to the 4 operators above, /sc-hacks/ uses 2 operators dedicated to connecting players through audio-busses.  These are:

1. =@>= : Connect the output of the receiver with the input of the argument with an allocated audio bus.  Adverb, if present, overrides the default name of the argument's input (=\in=).
2. =<@= : Connect the output of the argument with the input of the receiver with an allocated audio bus.  Adverb, if present, overrides the default name of the argument's output (=\out=).
