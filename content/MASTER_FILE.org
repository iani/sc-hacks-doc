...
Note: One can specify a custom output parameter for the reader player, with the =@= operator.

The adverb specifies custom input parameter for the argument player.
