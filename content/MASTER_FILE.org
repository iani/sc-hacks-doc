...
: An EventPattern is created from an Event.

#+BEGIN_SRC javascript;
~eventPattern = EventPattern(());
#+END_SRC

=EventPattern= instances can be played directly by sending the message =play=.  To play, an =EventPattern= instance creates an =EventStream= instance, and then plays that instance.
